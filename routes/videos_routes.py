...
class SingleVideo(MethodView):
    ...
    def put(self, id):
        ...
        if video:
            ...
            video.video_status.current_play_time = video_data['video_status']['current_play_time']
            video.video_status.play_count = video_data['video_status']['play_count']
            video.video_status.selection_count = video_data['video_status']['selection_count']
            video.video_status.is_watch_later = video_data['video_status']['is_watch_later']
...
class VideoRandom(MethodView):
    ...
    def get(self, filter_args):
        if "played" in filter_args:
            random_video = Video.query.join(VideoStatus).filter(VideoStatus.played == False).order_by(func.random()).first()
