...
class Requests {
    ...
    static async getVideo(ipAddress: string, video: Video) {
        const response = await fetch(`http://${ipAddress}:5000/videos/${video.id}`)
        return await response.json()
    }

    static async getRandomVideo(ipAddress: string, emit: any, query?: string) {
        const response = await fetch(`http://${ipAddress}:5000/videos/random${query || ''}`)
    ...
    static async getVideoContext(ipAddress: string) {
        try {
            const response = await fetch(`http://${ipAddress}:5000/video-context`)
